...
function hud(){
 ...
 $('weaponlevel').textContent=`${(s.machine||'m1a4').toUpperCase()} · LV.${s.level} · ${{ap:'穿甲',he:'榴彈',standard:'通用'}[s.ammo||'standard']}`;$('weaponname').textContent=s.arena&&s.level>=12?(s.level>=20?'貫穿副砲':s.level>=16?'強化副砲':'協同副砲'):s.machine==='m4a3'?'120mm 滑膛砲':s.machine==='xm2'?'88mm 滑膛砲':'57mm 滑膛砲';$('dots').innerHTML=Array.from({length:8},(_,i)=>`<i class="${i<s.level?'on':''}"></i>`).join('');$('combo').innerHTML=s.combo>=8?`${s.combo}<small>連續擊破</small>`:'';
