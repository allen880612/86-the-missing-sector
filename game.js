...
function start(retry=false){
 ...
 if(!retry||!runConfig)runConfig={machine:$('machine').value,encounter:$('encounter').value,ammo:$('ammo').value,tactic:$('tactic').value,bossModel:$('arenaBoss')?.value||'dinosauria',map:$('arenaMap')?.value||'ruins',mode:$('gameMode')?.value==='endless'?'endless':'frontline',seed:Number($('seed').value)||42};
...
function showResult(preview=false){
 ...
 $('reportLoadout').textContent=`${model} · LV.${r.level} / ${{ap:'穿甲彈',he:'榴彈',standard:'通用彈'}[r.ammo]} / ${r.tactic==='decoy'?'誘餌':'支援砲擊'}`;if(r.mode==='endless'){$('reportObjective').textContent='無限戰線';$('reportObjectiveState').textContent='第 '+r.wave+' 波';$('nextEncounter').hidden=true;}else $('nextEncounter').hidden=false;window.FrontlineUI?.reportDialogue?.('lena',r.won?'收到。作戰完成，你們辛苦了。':'紀錄已收到。重新整備，我們再找一條路。');if(!preview)$('reportTitle').focus();hud();
}
...
function hud(){
 ...
 if(s.arena)$('tip').textContent=s.uplinkBlocked?'火力座標受阻 · 擊落飛群或離開範圍':s.jammed?'僚機索敵受限 · 擊落阻電機群':'靠近補給即可回收';
