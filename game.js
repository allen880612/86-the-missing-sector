...
window.render_game_to_text=()=>JSON.stringify({mode:state,phaseRemaining:phaseTime,result:runResult,config:runConfig,map:s.mapId,worldBounds:s.worldBounds,bounds:s.bounds,weaponSlot:s.weaponSlot,specialAmmo:s.specialAmmo,coordinates:s.arena?'平面世界 x/y，範圍見 worldBounds；x向右 y向下，鏡頭固定1000視野':'x 0–1000 向右，y 0–1000 向下；小隊命中中心 y805',time:s.time,player:{arena:!!s.arena,y:s.y,aimX:s.aimX,aimY:s.aimY,angle:s.angle,mode:s.mode,wave:s.wave,threatStage:s.threatStage,wings:s.wings,machine:s.machine,ammo:s.ammo,charges:s.charges,brace:s.brace,momentum:s.momentum,jammed:s.jammed,x:s.x,hp:s.hp,won:s.won,count:s.count,shield:s.shield,level:s.level,overdrive:s.overdrive},enemies:s.enemies.map(e=>({id:e.id,type:e.type,model:e.model,bossStage:e.bossStage,x:e.x,y:e.y,hp:e.hp,max:e.max,charging:e.charging,chargeX:e.chargeX,attacks:e.attackCount,exposed:e.exposed,nodes:e.nodes,stagger:e.stagger||0,partDisabled:e.partDisabled})),bullets:s.bullets.map(b=>({x:b.x,y:b.y,vx:b.vx,vy:b.vy,shell:b.shell,enemy:b.enemy})),obstacles:s.obstacles,abilities:{burst:s.burstTime,burstCooldown:s.burstCooldown,dashCooldown:s.dashCooldown,dash:s.dash},render:{poseFrames:arenaPoseCache.size,poseBytes:arenaPoseCache.bytes,wrecks:wrecks.length,particles:particles.length,explosions:explosions.length},hazards:s.hazards,items:s.crates,gates:s.gates,stats:s.stats});
