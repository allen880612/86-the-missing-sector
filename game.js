...
let C=GameCore,audioActivated=false,trainingSession=null,trainingCaption='';
...
let phaseTime=0,phaseDuration=1.4,pausedFrom='playing',runConfig=null,pendingArenaWeapon=null,runResult=null,eventTime=0;
...
function processEvents(previousCrates=[]){
 if(trainingSession)GameTraining.observe(trainingSession,s.events.slice());
...
function resetSettlement(){document.body.classList.remove('settling');$('debrief').inert=false;$('debrief').classList.remove('incoming','revealed');$('debrief').style.removeProperty('--report-reveal');$('skipPhase').disabled=false;delete $('transition').dataset.beat;}
function start(retry=false){
 if(!assetReady)return;resetSettlement();window.FrontlineAudio?.stopEffects?.();window.FrontlineAudio?.unlock();beep('uiConfirm');window.manualTime=false;
 if(!retry&&!/^\d{1,6}$/.test(params.get('seed')||''))$('seed').value=1+Math.floor(Math.random()*999999);
 if(!retry||!runConfig)runConfig={machine:$('machine').value,encounter:$('encounter').value,ammo:$('ammo').value,tactic:$('tactic').value,bossModel:$('arenaBoss')?.value||'dinosauria',map:$('arenaMap')?.value||'ruins',mode:['endless','campaign'].includes($('gameMode')?.value)?$('gameMode').value:'frontline',seed:Number($('seed').value)||42};
 if(runConfig.mode==='campaign')runConfig={...runConfig,machine:'m1a4',tactic:'support'};
 C=runConfig.mode==='campaign'?GameCampaign:runConfig.mode==='endless'?GameArenaCore:GameCore;let seed=runConfig.seed;s=C.setup(C.create(()=>{seed=(seed*1664525+1013904223)>>>0;return seed/4294967296;}),runConfig);runResult=null;pendingArenaWeapon=null;state='launching';phaseTime=phaseDuration=2.6;keys.clear();drag=false;shellBursts=[];supportFx=[];railBursts=[];particles=[];texts=[];rings=[];impacts=[];explosions=[];effectsClock=0;lastExplosion=-1;wrecks=[];traces=[];shake=0;flash=0;danger=0;clearNotices();window.FrontlineUI?.phaseDialogue?.('lena','各機，準備出擊。我會支援你們。');
 ...
 $('transition').hidden=false;$('transition').className='sortie-transition launching';$('phaseLabel').textContent='SPEARHEAD / 出擊準備';$('phaseTitle').textContent=C.machines[s.machine].name;$('phaseDetail').textContent={pursuit:'誘導獵兵突進 · 閃開後反擊 · 後續重戰車',mixed:'突破軍團戰線 · 重戰車與軌道砲',dinosauria:'擊破重戰車 · 側移拆除副砲',morpho:'擊破電磁加速砲 · 避開致命射界',mines:'清理地雷 · 利用連鎖爆破','fire-support':'壓制火力支援 · 擊破斥候與砲兵'}[s.encounter];$('skipPhase').textContent='立即出擊';$('phaseProgress').style.width='0%';$('phaseMachine').src=previewSource(s.machine);$('phaseMachine').alt=C.machines[s.machine].name;const targetModel={'ash-return':'lowe',arena:'dinosauria',pursuit:'grauwolf',mixed:'dinosauria',dinosauria:'dinosauria',morpho:'morpho',mines:'mine','fire-support':'scorpion'}[s.encounter];$('phaseEnemy').src=previewSource(targetModel);$('phaseEnemy').alt={lowe:'Löwe',grauwolf:'Grauwolf',dinosauria:'Dinosauria',morpho:'Morpho',mine:'自走地雷',scorpion:'Skorpion'}[targetModel];$('phaseEnemyName').textContent=$('phaseEnemy').alt;if(s.arena){$('phaseEnemy').src=previewSource(runConfig.bossModel);$('phaseEnemy').alt=runConfig.bossModel;$('phaseDetail').textContent='四面接敵 · WASD 移動 · 自動主砲 · 左鍵機型技能／右鍵躍進';$('phaseEnemyName').textContent='軍團包圍網';$('tip').textContent='靠近補給即可回收';}else $('tip').textContent='走位選補給';if(s.campaign){$('phaseTitle').textContent='灰燼歸路';$('phaseEnemy').src=previewSource('lowe');$('phaseEnemyName').textContent='Löwe · 通路封鎖';$('phaseDetail').textContent='取回物資，穿越封鎖撤回。西側救援為可選任務。';window.FrontlineUI?.phaseDialogue?.('lena','回收點在舊城中央。保留撤離用的彈藥。');}canvas.style.cursor=s.arena?'crosshair':'';canvas.focus();hud();
}
function configure(){if(trainingSession){GameTraining.dispose(trainingSession);trainingSession=null;$('trainingPanel').hidden=true;document.body.classList.remove('training-mode');}resetSettlement();arenaPoseCache.clear();C=GameCore;state='ready';window.FrontlineAudio?.stopEffects?.();beep('uiSelect');keys.clear();drag=false;shellBursts=[];supportFx=[];railBursts=[];shake=0;particles=[];rings=[];impacts=[];texts=[];wrecks=[];explosions=[];traces=[];flash=0;muzzle=0;danger=0;clearNotices();$('transition').hidden=true;$('debrief').hidden=true;$('overlay').hidden=false;$('overlay').className='overlay';$('loadout').hidden=false;$('result').hidden=true;$('title').innerHTML='出擊<span>整備</span>';$('description').textContent='選擇你的機體，突破軍團戰線。';$('start').innerHTML='開始作戰 <span>→</span>';$('start').disabled=!assetReady;$('pause').disabled=true;$('assetstatus').hidden=false;$('hint').textContent='左右移動 · 自動砲擊 · 走位選補給';loadoutPreview();window.FrontlineUI?.refreshBriefing();preview();hud();}
$('configure').onclick=configure;
function startTraining(){
 if(!assetReady){toast('正在準備訓練素材，請稍候再試。');return;}
 configure();window.FrontlineAudio?.unlock();window.manualTime=false;C=GameArenaCore;trainingSession=GameTraining.create(C,{machine:$('machine').value});s=trainingSession.state;arenaPointer=null;state='playing';pendingArenaWeapon=null;trainingCaption='';
 $('titleScreen').hidden=true;$('overlay').hidden=true;$('overlay').inert=false;document.querySelector('.actions').inert=false;document.body.classList.remove('title-open');document.body.classList.add('training-mode','endless-mode');$('transition').hidden=true;$('debrief').hidden=true;$('trainingPanel').hidden=false;$('pause').disabled=false;$('footerControls').textContent='WASD 移動 · 游標瞄準 · 自動主砲 · 右鍵躍進 · Q 切換 · Space 支援';canvas.setAttribute('aria-label','四向實戰練習，WASD移動，游標瞄準，右鍵躍進，Q切換武器，Space支援');canvas.focus();updateTraining(0);hud();
}
function updateTraining(dt){
 if(!trainingSession)return;const old=s;s=GameTraining.tick(trainingSession,dt);
 if(s!==old){keys.clear();pendingArenaWeapon=null;arenaPointer=null;clearNotices();particles=[];texts=[];wrecks=[];traces=[];explosions=[];rings=[];impacts=[];}
 const lesson=trainingSession,caption=lesson.step+lesson.message+lesson.done;
 if(trainingCaption!==caption){trainingCaption=caption;$('trainingStep').textContent=lesson.done?'FIELD TRAINING / COMPLETE':`FIELD TRAINING / ${lesson.step+1} OF 5`;$('trainingTitle').textContent=lesson.done?'實戰練習完成':lesson.title;$('trainingObjective').textContent=lesson.done?'已練習走位、武器回收與反擊。返回整備，選擇正式作戰。':lesson.objective;$('trainingMessage').textContent=lesson.message;$('trainingPortrait').src=`assets/characters/${lesson.speaker==='lena'?'lena':'shin'}-reference.png`;$('trainingPortrait').alt=lesson.speaker==='lena'?'蕾娜':'辛';$('trainingExit').textContent=lesson.done?'返回整備':'離開練習';$('trainingRetry').textContent=lesson.done?'重新練習':'重試本項';$('trainingProgress').style.width=(lesson.done?100:lesson.step*20)+'%';beep(lesson.done?'uiConfirm':'uiSelect');}
 if(lesson.done){state='training-complete';keys.clear();$('pause').disabled=true;}
}
$('trainingExit').onclick=()=>{beep('uiSelect');configure();$('start').focus();};
$('trainingRetry').onclick=()=>{if(!trainingSession)return;if(trainingSession.done){startTraining();return;}s.over=true;updateTraining(0);canvas.focus();};
for(const button of document.querySelectorAll('#trainingPanel button'))button.onpointerenter=()=>beep('uiHover');
window.addEventListener('training-start',startTraining);
...
function loadoutPreview(){
 if($('gameMode').value!=='frontline'&&window.GameArenaCore){arenaLoadoutPreview();return;}
 const C=GameCore;
...
function useTactic(){if(state!=='playing')return;s.events=[];C.useTactic(s);processEvents();hud();}
...
function interactCampaign(){if(state!=='playing'||!s.campaign)return;s.events=[];C.interact(s);processArenaEvents();s.events=[];hud();}
$('missionInteract').onclick=()=>{interactCampaign();canvas.focus();};
function end(){
 if(trainingSession)return;
 if(runResult)return;runResult=C.result(s);if(!runResult)return;state='settling';phaseTime=phaseDuration=s.won?3.2:2.8;keys.clear();drag=false;pendingArenaWeapon=null;texts=[];clearNotices();shake=flash=0;$('overlay').hidden=true;$('pause').disabled=true;document.body.classList.add('settling');window.FrontlineUI?.phaseDialogue?.('shin',s.won?(s.campaign?'物資已帶回。各機，返回防線。':'目標沉默。回收隊形，準備撤離。'):'主機失能。紀錄傳送中。');
 $('transition').className='sortie-transition settling '+(s.won?'victory':'defeat');$('phaseLabel').textContent=s.won?'MISSION COMPLETE':'SIGNAL LOST';$('phaseTitle').textContent=s.won?'戰線突破':'主機訊號中斷';$('phaseDetail').textContent=s.won?(s.campaign?'回收確認 · 返回防線':'戰果確認中'):'作戰紀錄已保存';if(s.campaign&&s.won)$('phaseTitle').textContent='撤離完成';$('skipPhase').textContent='查看戰報';$('skipPhase').disabled=true;$('phaseProgress').style.width='0%';if(!s.won&&motion)burst(s.x,s.arena?s.y:792,'#a68d78',12,true);updateSettlement();$('transition').hidden=false;beep(s.won?'victory':'defeat');
}
function updateSettlement(){
 const elapsed=phaseDuration-phaseTime,reduced=!motion||matchMedia('(prefers-reduced-motion: reduce)').matches;$('skipPhase').disabled=elapsed<.25;
 if(s.won)return;
 const transition=$('transition'),black=reduced?.82:elapsed<.3?.18+.09*elapsed/.3:elapsed<1.2?.27+.27*(elapsed-.3)/.9:.54+.30*clamp((elapsed-1.2),0,1),red=reduced?0:elapsed<.3?.22*elapsed/.3:elapsed<1.2?.22:.22*(1-clamp(elapsed-1.2,0,1));
 transition.style.setProperty('--settle-black',black);transition.style.setProperty('--settle-red',red);transition.style.setProperty('--settle-copy',reduced?(elapsed<2.2?1:0):clamp((elapsed-.3)/.25,0,1)*(1-clamp((elapsed-2.2)/.25,0,1)));transition.dataset.beat=elapsed<.3?'impact':elapsed<1.2?'signal':elapsed<2.2?'blackout':'report';
 if(elapsed>=2.2){if($('debrief').hidden)showResult(true);$('debrief').style.setProperty('--report-reveal',reduced?1:clamp((elapsed-2.2)/.6,0,1));}
}
function showResult(preview=false){
 if(!runResult)return;const visible=!$('debrief').hidden;if(!preview){state='over';$('transition').hidden=true;document.body.classList.remove('settling');}$('debrief').hidden=false;$('debrief').inert=preview;$('debrief').className='debrief '+(runResult.won?'victory':'defeat')+(preview?' incoming':'')+(preview||visible?' revealed':'');if(!preview)$('debrief').style.removeProperty('--report-reveal');if(visible){if(!preview)$('reportTitle').focus();return;}
 ...
 $('reportStats').innerHTML=[['作戰時間',formatTime(r.time)],['確認擊破',r.kills],['武裝打斷',r.interrupts],['承受傷害',Math.round(r.damage)]].map(([name,value])=>`<div><span>${name}</span><strong>${value}</strong></div>`).join('');
 $('reportCode').textContent=r.won?'MISSION COMPLETE / 作戰紀錄':'SIGNAL LOST / 作戰紀錄';$('reportObjective').textContent={'ash-return':'灰燼歸路',arena:'無限戰線',pursuit:'獵兵追擊 → 重戰車',mixed:'重戰車 → 電磁加速砲',dinosauria:'Dinosauria · 重戰車',morpho:'Morpho · 電磁加速砲',mines:'地雷連鎖','fire-support':'火力支援壓制'}[r.encounter];$('reportEnemy').src=previewSource({'ash-return':'lowe',pursuit:r.won?'dinosauria':'grauwolf',mixed:r.bossKills===0?'dinosauria':'morpho',dinosauria:'dinosauria',morpho:'morpho',mines:'mine','fire-support':'scorpion',arena:s.enemies.find(e=>e.type==='boss'&&!e.dead)?.model||runConfig?.bossModel||'dinosauria'}[r.encounter]);$('reportEnemy').alt=$('reportObjective').textContent;$('reportObjectiveState').textContent=r.encounter==='mixed'?`${r.bossKills} / 2 擊破`:r.won?'已完成':'未完成';$('reportResources').innerHTML=[['頭目擊破',r.bossKills],['補給取得',r.items],['戰術使用',r.tactics],...(r.mode==='endless'?[['僚機攔截',r.sacrifices||0]]:[])].map(([label,value])=>`<span>${label} <b>${value}</b></span>`).join('');
 const cause=r.lastDamage,causes={blade:['鏈刃橫掃','離開正面扇區或越障躍進，等收刃再反擊。'],ram:['衝撞','等敵機鎖定後橫移，讓獵兵撲空。'],contact:['敵機突破','優先清理接近主機的敵機。'],sweep:['電磁加速砲封鎖','提早進入射界間的缺口。'],rail:[cause?.lethal?'致命主砲':'預鎖主砲','砲口鎖定後橫移，讓主機中心離開射界。'],heavy:['重型主砲','砲口鎖定後換線，利用後座窗口反擊。'],secondary:['副砲齊射','站到側面，拆除對應砲座。'],vulcan:['火神砲掃射','鎖定後換線，保留戰術支援。'],mortar:['砲擊爆區','離開落點後別立即折返；優先擊破斥候。'],mine:['自走地雷引爆','提前射爆地雷，利用連鎖清場。']};
 const sourceName={dinosauria:'Dinosauria',morpho:'Morpho',phoenix:'Phönix',stier:'Stier',gunner:'Ameise 掃射班',grauwolf:'Grauwolf',charger:'Grauwolf',artillery:'Skorpion',mine:'自走地雷',shield:'Löwe',normal:'Ameise',scout:'Ameise',boss:'Löwe',swarm:'Eintagsfliege',jammer:'Eintagsfliege'}[cause?.model]||'';
 ...
 const next={mixed:'pursuit',pursuit:'fire-support','fire-support':'mines',mines:'dinosauria',dinosauria:'morpho',morpho:'mixed',arena:'mixed','ash-return':'mixed'}[r.encounter];
 ...
 $('reportLoadout').textContent=`${model} · LV.${r.level} / ${{ap:'穿甲彈',he:'榴彈',standard:'通用彈'}[r.ammo]} / ${r.tactic==='decoy'?'誘餌':'支援砲擊'}`;if(r.mode==='endless'){$('reportObjective').textContent='無限戰線';$('reportObjectiveState').textContent='第 '+r.wave+' 波';$('nextEncounter').hidden=true;}else $('nextEncounter').hidden=false;if(r.mode==='campaign'){$('nextEncounter').hidden=true;$('reportTitle').textContent=r.won?'撤離完成':'作戰中止';$('reportSummary').textContent=r.won?'物資已回收，小隊返回防線。'+r.summary:'主機失能。回收紀錄已保存，重新整備後再出擊。';$('reportObjective').textContent='灰燼歸路';$('reportObjectiveState').textContent=r.cargo?'物資已回收':'物資未回收';$('reportResources').innerHTML=`<span>物資 <b>${r.cargo?'已回收':'未取得'}</b></span><span>西側救援 <b>${r.rescued?'完成':'未完成'}</b></span><span>確認擊破 <b>${r.kills}</b></span>`;$('reportFootnote').textContent='重新挑戰會重置任務、敵人與補給；撤離成功不要求擊毀所有敵機。';}window.FrontlineUI?.reportDialogue?.('lena',r.won?'收到。作戰完成，你們辛苦了。':'紀錄已收到。重新整備，我們再找一條路。');if(!preview)$('reportTitle').focus();hud();
}
...
$('skipPhase').onclick=()=>{if(state==='settling'&&phaseDuration-phaseTime<.25)return;completePhase();};$('retry').onclick=()=>start(true);$('changeLoadout').onclick=configure;
...
function queueArenaWeapon(slot){if(!s.arena||state!=='playing'||$('commandDialog')?.open)return;pendingArenaWeapon=slot;}
for(const button of document.querySelectorAll('[data-weapon-slot]'))button.addEventListener('click',()=>queueArenaWeapon(Number(button.dataset.weaponSlot)));
window.addEventListener('keydown',e=>{if($('commandDialog')?.open)return;if(state==='ready'||state==='over'||state==='settling')return;if(['INPUT','SELECT'].includes(e.target.tagName))return;if(s.arena&&state==='playing'&&['Digit1','Digit2','Digit3','KeyQ'].includes(e.code)){e.preventDefault();if(!e.repeat){if(trainingSession&&e.code==='KeyQ')GameTraining.observe(trainingSession,{type:'trainingInput',code:'KeyQ'});const available=[1,s.specialAmmo.autocannon>0?2:0,s.specialAmmo.heavy>0?3:0].filter(Boolean);queueArenaWeapon(e.code==='KeyQ'?available[(available.indexOf(s.weaponSlot)+1)%available.length]:Number(e.code.slice(-1)));}return;}if(e.code==='KeyE'&&s.campaign&&state==='playing'){e.preventDefault();if(!e.repeat)interactCampaign();return;}if(e.code==='Space'){if(e.target.tagName==='BUTTON')return;e.preventDefault();if(!e.repeat)useTactic();return;}if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','a','A','d','D','w','W','s','S','p','P'].includes(e.key)){e.preventDefault();keys.add(e.key.toLowerCase());if(e.key.toLowerCase()==='p'&&!e.repeat)pause();}});
window.addEventListener('command-dialog-open',()=>{keys.clear();drag=false;});
...
function supplyPreview(box){
 ...
 if(box.kind==='repair'){const gain=n(s.arena?12:30,s.maxHp-s.hp);return [gain?'耐久 +'+Math.ceil(gain):'耐久已滿','修復主機',!gain];}
 if(box.kind==='shield'){const gain=n(s.arena?10:box.drop?20:35,s.maxHp-s.shield);return [gain?'裝甲 +'+Math.ceil(gain):'裝甲已滿','吸收一般傷害',!gain];}
 ...
 if(box.kind==='weapon'&&s.arena)return ['LV.'+s.level+' → '+(s.level+1),s.level<12?'LV.12 解鎖協同副砲':s.level<16?'LV.16 強化副砲':s.level<20?'LV.20 副砲貫穿':'持續強化砲擊',false];
...
function draw(){
 ...
 if(trainingSession&&!trainingSession.done&&trainingSession.marker&&trainingSession.step<2){const m=trainingSession.marker,p=arenaPoint(m.x,m.y);ctx.save();ctx.strokeStyle='#b5ddd0';ctx.lineWidth=2;ctx.setLineDash([7,5]);ctx.beginPath();ctx.ellipse(p.x,p.y,m.r*W*.00086,m.r*H*.00084,0,0,Math.PI*2);ctx.stroke();ctx.setLineDash([]);label('目的地',p.x,p.y-12,13,'#d0eee2',600);ctx.restore();}
}
function update(dt){
...
for(const fx of fireLinkFx)fx.age+=dt;fireLinkFx=fireLinkFx.filter(fx=>fx.age<.4);
 ...
 if(state==='launching'||state==='settling'){phaseTime-=dt;if(state==='settling')updateSettlement();$('phaseProgress').style.width=`${clamp(1-phaseTime/phaseDuration,0,1)*100}%`;if(phaseTime<=0)completePhase();}
 ...
 s.muzzleScale=Math.max(65,W*.105)*.8/(H*.00080);s.muzzleY=792-s.muzzleScale*point(500,792).scale;if(state==='playing'){if(s.arena)arenaRefreshAim();const previousCrates=s.crates.slice();C.step(s,dt,s.arena?{moveX:Number(keys.has('d')||keys.has('arrowright'))-Number(keys.has('a')||keys.has('arrowleft')),moveY:Number(keys.has('s')||keys.has('arrowdown'))-Number(keys.has('w')||keys.has('arrowup')),aimX:s.aimX,aimY:s.aimY,firing:true,weaponSlot:pendingArenaWeapon??undefined}:target);pendingArenaWeapon=null;if(s.arena)arenaRefreshAim();refreshBattleRadio();processEvents(previousCrates);if(trainingSession)updateTraining(dt);refreshBattleRadio();if(!activeRadio)coreRadio(s.enemies.find(e=>e.type==='boss'&&!e.dead&&e.exposed>0));}if(s.over&&state==='playing')end();for(const e of wrecks){e.life-=dt;if(e.duration>1){const age=e.duration-e.life,stages=e.kind==='boss'?(s.arena?[.16,.42]:[.16,.42,.72]):[.18];while(state==='playing'&&e.burstStage<stages.length&&age>=stages[e.burstStage]){const i=e.burstStage++,x=e.x+(i===0?-45:i===2?40:0),y=e.y-(i===1?50:25);blastEffect(x,y,e.kind==='boss'?(i===1?170:92):75,e.kind==='boss'&&i===1,i===1?.95:.5,true);burst(x,y,'#d7b895',i===1?25:9,true);if(i===1){ring(x,y,'#d9b28c',true);if(motion)shake=Math.max(shake,5);}}}}wrecks=wrecks.filter(e=>e.life>0);
...
function hud(){
 $('arenaWeapons').hidden=!s.arena||!['playing','paused'].includes(state);
 if(s.arena)for(const button of document.querySelectorAll('[data-weapon-slot]')){const slot=Number(button.dataset.weaponSlot),ammo=slot===1?Infinity:slot===2?s.specialAmmo.autocannon:s.specialAmmo.heavy;button.disabled=state!=='playing'||ammo<=0;button.classList.toggle('active',slot===s.weaponSlot);button.setAttribute('aria-pressed',String(slot===s.weaponSlot));button.querySelector('span').textContent=slot===1?'∞':ammo+' / '+(slot===2?120:18);button.setAttribute('aria-label',(slot===1?'主砲無限彈藥':slot===2?'機砲剩餘 '+ammo+' 發':'穿甲重砲剩餘 '+ammo+' 發'));}
 $('arenaAbilities').hidden=!s.arena||!['playing','paused'].includes(state);
 if(s.arena){
  const skillName={m1a4:'近距反擊',m4a3:'重砲超頻',xm2:'高速刃擊'}[s.machine];$('arenaBurst').querySelector('b').textContent=skillName;$('arenaBurst').setAttribute('aria-label',`左鍵${skillName}，冷卻${s.machine==='m1a4'?8:9}秒`);
  for(const [id,remaining,active] of [['arenaBurst',s.burstCooldown||0,s.burstTime||0],['arenaDash',s.dashCooldown||0,s.dash?.remaining||0]]){
   const button=$(id);button.disabled=state!=='playing'||remaining>0;button.classList.toggle('active',active>0);button.querySelector('span').textContent=active>0?active.toFixed(1)+'s':remaining>0?remaining.toFixed(1)+'s':'就緒';button.style.setProperty('--ready',String(1-remaining/(id==='arenaBurst'?(s.machine==='m1a4'?8:9):3)));
  }
 }
 ...
 const bosses=s.enemies.filter(e=>e.type==='boss'&&!e.dead),boss=bosses[0];$('threat').textContent=s.over?(s.won?'作戰完成':'訊號中斷'):boss?'重型目標接敵':s.jammed?'火控干擾':s.stage===1?`軌道砲接敵 ${Math.max(0,Math.ceil(s.boss))}s`:'壁外作戰';if(s.mode==='endless')$('threat').textContent=s.over?'無限戰線結束':`第 ${s.wave} 波 · ${s.wavePhase==='regroup'?'整補 '+Math.ceil(s.regroupRemaining||0)+'s':s.wavePhase==='cleanup'?'清場中 · 殘敵 '+s.enemies.filter(e=>!e.dead&&!e.retreating).length:'增援 '+(s.waveSpawned||0)+'／'+(s.waveTotal||0)}${s.waveKind==='boss'?' · Boss 強襲':s.waveKind==='elite'?' · 菁英強襲':''}`;const total=s.mode==='endless'?Math.max(1,s.bossKills+1):s.encounter==='mixed'?2:1,progress=clamp((s.bossKills+(boss?1-boss.hp/boss.max:0))/total,0,1)*100;$('threatbar').style.width=(s.arena?clamp(((s.waveSpawned||0)-s.enemies.filter(e=>!e.dead&&!e.retreating).length)/Math.max(1,s.waveTotal||1),0,1)*100:progress)+'%';$('threatbar').parentElement.setAttribute('aria-valuenow',Math.round(parseFloat($('threatbar').style.width)));$('objectiveRoute').hidden=s.mode==='endless'||s.encounter!=='mixed';$('routeDino').className=s.bossKills>0?'done':'current';$('routeMorpho').className=s.bossKills>1?'done':s.stage===1?'current':'';$('objectiveRoute').setAttribute('aria-label',`重戰車${s.bossKills>0?'已擊破':'待擊破'}，電磁加速砲${s.bossKills>1?'已擊破':s.stage===1?(boss?'接戰中':'接近中'):'待突破'}`);
 $('weaponlevel').textContent=`${(s.machine||'m1a4').toUpperCase()} · LV.${s.level} · ${{ap:'穿甲',he:'榴彈',standard:'通用'}[s.ammo||'standard']}`;$('weaponname').textContent=s.arena&&s.weaponSlot>1?(s.weaponSlot===2?'連發機砲':'穿甲重砲'):s.arena&&s.level>=12?(s.level>=20?'貫穿副砲':s.level>=16?'強化副砲':'協同副砲'):s.machine==='m4a3'?'120mm 滑膛砲':s.machine==='xm2'?'88mm 滑膛砲':'57mm 滑膛砲';$('dots').innerHTML=Array.from({length:8},(_,i)=>`<i class="${i<s.level?'on':''}"></i>`).join('');$('combo').innerHTML=s.combo>=8?`${s.combo}<small>連續擊破</small>`:'';
 ...
 $('tactical').disabled=state!=='playing'||!s.charges||s.tacticCooldown>0||!!s.uplinkBlocked;$('tactical').innerHTML=`${s.uplinkBlocked?'鏈路受擾':s.tactic==='decoy'?'放置誘餌':'支援砲擊'}<small>空白鍵 · ${s.charges||0} 次${s.arena&&s.charges<2?' · '+Math.ceil(s.tacticRecharge||0)+'秒回充':''}</small>`;
 ...
 $('bossWave').hidden=!s.arena;$('bossWave').textContent=s.arena?`第 ${s.wave} 波 ／ 威脅 ${s.threatStage||1}`:'';
 $('bossContacts').hidden=!s.arena||bosses.length<2;$('bossContacts').innerHTML=s.arena&&bosses.length>1?bosses.map(e=>`<span class="boss-contact" data-phase="${e.bossStage||1}"><b>${{dinosauria:'重戰車',morpho:'電磁砲',phoenix:'高機動'}[e.model]||'重型'}${e.bossStage>=2?' II':''}</b><i style="--hp:${clamp(e.hp/e.max*100,0,100)}%"></i><small>${Math.ceil(e.hp/e.max*100)}%</small></span>`).join(''):'';
 if(boss){
  $('bossname').textContent=boss.model==='dinosauria'?'DINOSAURIA · 重戰車型':boss.model==='morpho'?'MORPHO · 電磁加速砲型':boss.model==='phoenix'?'PHÖNIX · 高機動型':'LÖWE · 戰車型';$('bosshp').textContent=`${s.arena&&boss.bossStage>=2?'II · ':''}${Math.ceil(boss.hp/boss.max*100)}%`;$('bossfill').style.width=`${clamp(boss.hp/boss.max*100,0,100)}%`;
  const pending=s.hazards.filter(h=>h.source===boss.id&&!h.fired).sort((a,b)=>a.delay-b.delay)[0],firing=s.hazards.some(h=>h.source===boss.id&&h.fired&&h.life>0&&(!s.arena||['beam','sector'].includes(h.geometry))),permanent=!s.arena&&boss.model==='dinosauria'&&boss.nodes.every(n=>n<=0),phase=s.arena&&boss.exposed>0?'exposed':firing?'firing':boss.exposed>0?'exposed':pending?'charging':'armored';
  $('bossbar').dataset.phase=phase;$('bossbar').dataset.model=boss.model||'lowe';$('bosshint').textContent=s.arena?(boss.model==='phoenix'?(boss.phase==='stalk'&&!(boss.revealed>0)?'光學迷彩 · 訊號失聯':boss.exposed>0?'收刃破綻 · 反擊 ×1.6':boss.phase==='slashWindup'?'鏈刃展開 · 離開正面':boss.phase==='slash'?'鏈刃橫掃':boss.phase==='dash'?'高速切入':boss.phase==='windup'?'鎖定完成 · 側向躍進':boss.revealed>0?'側翼機動 · 迷彩破除':'側翼追蹤 · 射擊顯形'):boss.exposed>0?'散熱窗口 · 反擊 ×1.6':firing?'火力釋放':pending?(pending.geometry==='beam'?'射界鎖定 · 移出光帶':'落點鎖定 · 離開圓圈'):'追蹤中'):firing?'砲擊中':boss.exposed>0?(permanent?'核心持續暴露':'核心開放 · '+boss.exposed.toFixed(1)+'s'):pending?({heavy:'主砲鎖定',secondary:'副砲齊射',sweep:'電磁蓄能',rail:'主砲預鎖',vulcan:'火神砲鎖定'}[pending.kind]||'武裝蓄力'):'裝甲閉合';
  const progress=phase==='exposed'?(permanent?1:clamp(boss.exposed/(s.arena?(boss.model==='phoenix'?(boss.recoverDuration||(s.wave<=3?2.2:1.8)):2):boss.model==='morpho'?2.2:1.5),0,1)):pending?clamp(1-pending.delay/pending.maxDelay,0,1):0;
 ...
 $('missionPanel').hidden=!s.campaign||!['playing','paused'].includes(state);
 if(s.campaign){const m=s.campaign,near=m.nodes.find(n=>!n.done&&Math.hypot(n.x-s.x,n.y-s.y)<=n.r+22);$('missionObjective').textContent=m.objective;$('missionHint').textContent=m.hint;$('missionInteract').hidden=!near||near.id==='exit';$('missionInteract').disabled=state!=='playing';$('missionInteract').innerHTML='<kbd>E</kbd> '+(m.working?'取消工作':near?.id==='rescue'?'開始救援':'開始回收');$('missionProgress').style.width=(m.working?clamp(m.progress,0,1)*100:m.cargo?100:0)+'%';$('threat').textContent=m.cargo?'物資回收完成 · 前往北側撤離':'灰燼歸路 · 回收作戰';$('threatbar').style.width=(m.cargo?70:15)+'%';}
 if(s.campaign)$('tip').textContent='E 回收／取消 · 北側撤離';else if(s.arena)$('tip').textContent=s.uplinkBlocked?'火力座標受阻 · 擊落飛群或離開範圍':s.jammed?'僚機索敵受限 · 擊落阻電機群':'靠近補給即可回收';
...
preview();resize();hud();
function frame(now){const dt=Math.min(.033,(now-previous)/1000||0);previous=now;if(['launching','playing','settling','training-complete'].includes(state)&&!window.manualTime&&!$('commandDialog')?.open)update(dt);if(state==='playing')battleMusicScene=s.enemies.some(e=>e.type==='boss')?'boss':'battle';window.FrontlineAudio?.setScene?.(!$('titleScreen').hidden?'title':['ready','launching'].includes(state)?'prep':state==='settling'&&phaseDuration-phaseTime<(s.won?1.1:2.2)?battleMusicScene:['over','settling'].includes(state)?s.won?'victory':'defeat':state==='paused'&&pausedFrom==='launching'?'prep':battleMusicScene);window.FrontlineAudio?.update({playing:['launching','playing','settling'].includes(state),phase:state,musicFade:state==='settling'&&!s.won?(phaseDuration-phaseTime<2.2?Math.max(0,1-(phaseDuration-phaseTime)/2.2):.56*clamp((phaseDuration-phaseTime-2.2)/.6,0,1)):undefined,boss:s.enemies.some(e=>e.type==='boss'),elite:s.enemies.some(e=>['scout','shield','charger'].includes(e.type)),hp:s.hp??100,won:s.won});draw();if(now-lastHud>90){hud();lastHud=now;}requestAnimationFrame(frame);}requestAnimationFrame(frame);

window.advanceTime=ms=>{window.manualTime=true;for(let i=0;i<Math.round(ms/1000*60);i++)if(['launching','playing','settling','training-complete'].includes(state))update(1/60);draw();hud();};
window.render_game_to_text=()=>JSON.stringify({mode:state,training:trainingSession?{step:trainingSession.step,done:trainingSession.done,title:trainingSession.title}:null,phaseRemaining:phaseTime,result:runResult,campaign:s.campaign||null,config:runConfig,map:s.mapId,worldBounds:s.worldBounds,bounds:s.bounds,weaponSlot:s.weaponSlot,specialAmmo:s.specialAmmo,coordinates:s.arena?'平面世界 x/y，範圍見 worldBounds；x向右 y向下，鏡頭固定1000視野':'x 0–1000 向右，y 0–1000 向下；小隊命中中心 y805',time:s.time,player:{arena:!!s.arena,y:s.y,aimX:s.aimX,aimY:s.aimY,angle:s.angle,mode:s.mode,wave:s.wave,waveKind:s.waveKind,wavePhase:s.wavePhase,waveSpawned:s.waveSpawned,waveTotal:s.waveTotal,regroupRemaining:s.regroupRemaining,uplinkBlocked:s.uplinkBlocked,threatStage:s.threatStage,wings:s.wings,machine:s.machine,ammo:s.ammo,charges:s.charges,brace:s.brace,momentum:s.momentum,jammed:s.jammed,x:s.x,hp:s.hp,won:s.won,count:s.count,shield:s.shield,level:s.level,overdrive:s.overdrive},enemies:s.enemies.map(e=>({id:e.id,type:e.type,model:e.model,bossStage:e.bossStage,phase:e.phase,revealed:e.revealed,x:e.x,y:e.y,hp:e.hp,max:e.max,charging:e.charging,chargeX:e.chargeX,attacks:e.attackCount,exposed:e.exposed,nodes:e.nodes,stagger:e.stagger||0,partDisabled:e.partDisabled})),bullets:s.bullets.map(b=>({x:b.x,y:b.y,vx:b.vx,vy:b.vy,shell:b.shell,enemy:b.enemy})),obstacles:s.obstacles,abilities:{burst:s.burstTime,burstCooldown:s.burstCooldown,dashCooldown:s.dashCooldown,dash:s.dash},render:{poseFrames:arenaPoseCache.size,poseBytes:arenaPoseCache.bytes,wrecks:wrecks.length,particles:particles.length,explosions:explosions.length},hazards:s.hazards,items:s.crates,gates:s.gates,stats:s.stats});
