...
window.addEventListener('keydown',e=>{if($('commandDialog')?.open)return;if(state==='ready'||state==='over'||state==='settling')return;if(['INPUT','SELECT'].includes(e.target.tagName))return;if(e.code==='Space'){if(e.target.tagName==='BUTTON')return;e.preventDefault();if(!e.repeat)useTactic();return;}if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','a','A','d','D','w','W','s','S','p','P'].includes(e.key)){e.preventDefault();keys.add(e.key.toLowerCase());if(e.key.toLowerCase()==='p'&&!e.repeat)pause();}});
