...
function showResult(){
 ...
 const info=causes[cause?.kind]||['敵方火力','觀察射界與敵機預備動作，再選擇反擊時機。'];$('reportCause').hidden=r.won;$('reportCause').innerHTML=`<span>致命來源</span><strong>${sourceName?sourceName+' · ':''}${info[0]}</strong><p>${info[1]}</p>`;
...
$('nextEncounter').onclick=()=>{if(state!=='over'||!runConfig)return;const seed=(runConfig.seed+1+Math.floor(Math.random()*999998)-1)%999999+1;runConfig={...runConfig,encounter:$('nextEncounter').dataset.encounter,seed};$('seed').value=seed;$('encounter').value=runConfig.encounter;$('encounter').dispatchEvent(new Event('change',{bubbles:true}));start(true);};
...
function rewardFeedback(e){
 ...
 else if(e.kind==='weapon')message=a.level>b.level?'火控 LV.'+a.level:s.arena?'火控已滿':'急速裝填 '+Math.ceil(a.overdrive)+'s';
...
function draw(){
 ...
 for(const hit of impacts){const p=point(hit.x,hit.y),f=hit.life/hit.max,r=(hit.style==='fracture'?32:hit.large?26:hit.style==='ricochet'?11:15)*p.scale;ctx.save();ctx.globalAlpha=f;ctx.translate(p.x,p.y);
  ...
  ctx.restore();}
 ctx.globalAlpha=1;for(const t of texts){const p=point(t.x,t.y);ctx.globalAlpha=Math.min(1,t.life*3);label(t.str,p.x,p.y,s.arena?14:24,t.color,900);}ctx.globalAlpha=1;if(s.arena){drawArenaReticle();}else{drawSupport();drawShellBursts();drawRailBursts();drawArmorBreaks();for(const box of s.crates)drawCrate(box,false,true);for(const box of supplyGhosts)drawCrate(box,true,true);drawHazards(true);drawPlayerMarker();drawFireControl();}ctx.restore();drawRewards();
...
function hud(){
 ...
 if(boss){
  ...
  const pending=s.hazards.filter(h=>h.source===boss.id&&!h.fired).sort((a,b)=>a.delay-b.delay)[0],firing=s.hazards.some(h=>h.source===boss.id&&h.fired&&h.life>0),permanent=boss.model==='dinosauria'&&boss.nodes.every(n=>n<=0),phase=firing?'firing':boss.exposed>0?'exposed':pending?'charging':'armored';
  $('bossbar').dataset.phase=phase;$('bossbar').dataset.model=boss.model||'lowe';$('bosshint').textContent=s.arena?(firing?'火力釋放':boss.exposed>0?'散熱窗口 · 反擊 ×1.6':pending?(pending.geometry==='beam'?'射界鎖定 · 移出光帶':'落點鎖定 · 離開圓圈'):'追蹤中'):firing?'砲擊中':boss.exposed>0?(permanent?'核心持續暴露':'核心開放 · '+boss.exposed.toFixed(1)+'s'):pending?({heavy:'主砲鎖定',secondary:'副砲齊射',sweep:'電磁蓄能',rail:'主砲預鎖',vulcan:'火神砲鎖定'}[pending.kind]||'武裝蓄力'):'裝甲閉合';
  const progress=phase==='exposed'?(permanent?1:clamp(boss.exposed/(boss.model==='morpho'?2.2:1.5),0,1)):pending?clamp(1-pending.delay/pending.maxDelay,0,1):0;
