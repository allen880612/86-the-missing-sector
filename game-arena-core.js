(function(root){
 ...
 const MAPS={
  ...
  rail:{name:'鐵路砲陣地',walls:[['tw1',410,500,300,100],['tw2',800,500,300,100],['tw3',1190,500,300,100],['tw4',440,1100,360,100],['tw5',830,1100,220,100],['tw6',1190,1100,300,100]],covers:[['tc1',510,350,55,140],['tc2',1090,660,55,140],['tc3',570,940,55,140],['tc4',1080,1270,55,140]],supplyAnchors:[[510,800],[1090,800],[570,250],[1080,1350],[800,800]]}
 };
 const WEAPONS={1:{kind:'main',max:null},2:{kind:'autocannon',max:120,damage:18,interval:.1,speed:900,r:4,range:650,left:2},3:{kind:'heavy',max:18,damage:120,interval:.62,speed:720,r:10,range:780,left:6}};
 ...
 const blocked=(s,body,x,y,others=true)=>{
  ...
  return (s.enemies||[]).some(e=>e!==body&&!e.dead&&!(body===s&&e.model==='phoenix'&&['stalk','windup','dash','slashWindup','slash','recover'].includes(e.phase))&&Math.hypot(x-e.x,y-e.y)<body.r+e.r-1e-6);
 };
 ...
 const dropKind=(s,quality='ordinary')=>quality==='boss'?['heavyCannon','autocannon','overdrive','emp'][s.bossDropCycle++%4]:quality==='elite'?['weapon','autocannon','heavyCannon','emp','overdrive','charge'][s.eliteDropCycle++%6]:['shield','repair','shield','repair','charge','repair'][s.dropCycle++%6];
 const bossCapForWave=wave=>1+(wave>=4)+(wave>=7)+(wave>=13)+(wave>=19);
 const waveKind=wave=>wave%10===0?'boss':wave%5===0?'elite':'normal';
 const wavePlan=wave=>{const kind=waveKind(wave);if(kind==='elite'){const total=Math.min(20,6+2*Math.floor((wave-5)/10));return {kind,total,bosses:0,elites:total};}if(kind==='boss'){const total=Math.min(5,3+Math.floor((wave-10)/10));return {kind,total,bosses:total,elites:0};}const nonboss=Math.min(28,6+2*wave),bosses=wave<=2?0:wave<=6?1:wave>=19?4:wave>=13?3:2,share=Math.min(.5,.2+Math.floor((wave-1)/3)*.05),elites=wave===1?1:wave===2?1:wave===3?2:Math.max(1,Math.round(nonboss*share));return {kind,total:nonboss+bosses,bosses,elites};};
 ...
 const C={
  ...
  create:(random=Math.random)=>({
   ...
   wingShot:0,spawn:1.3,boss:32,bossWarned:false,bossModel:'dinosauria',bossIndex:0,bossSpawnCount:0,bossCap:1,pickupTimer:8,spawnSide:0,wave:1,waveKind:'normal',wavePhase:'engaging',waveElapsed:0,waveSpawned:0,waveTotal:8,waveBossSpawned:0,waveEliteSpawned:0,regroupRemaining:0,threatStage:1,nextSpecial:16,specialIndex:0,nextFlockId:1,mainShotCount:0,wingRecruitTimer:45,eliteQueue:[],eliteIndex:0,
   charges:2,tacticCooldown:0,tacticRecharge:30,invulnerable:0,heavyInvulnerable:0,invulnerabilitySource:null,angle:-Math.PI/2,aimX:500,aimY:200,r:22,
   burstTime:0,burstCooldown:0,dashCooldown:0,dash:null,wingSaveCooldown:0,normalKills:0,lastEliteDrop:-Infinity,dropCycle:0,eliteDropCycle:0,bossDropCycle:0,supplyGroup:0,wingRange:280,jammed:false,uplinkBlocked:false,
  ...
  setup(s,options={}){
   ...
   s.hp=s.maxHp=machine.hp;s.shield=25;s.dash=null;s.dashCooldown=0;s.wingSaveCooldown=0;s.invulnerable=0;s.heavyInvulnerable=0;s.invulnerabilitySource=null;s.mode='endless';s.arena=true;s.encounter='arena';s.wings=[];s.nextWingId=1;s.uplinkBlocked=false;syncWings(s);
   s.ammo=['ap','he'].includes(options.ammo)?options.ammo:'standard';s.weaponSlot=1;s.specialAmmo={autocannon:0,heavy:0};
   s.tactic='support';s.charges=2;s.tacticRecharge=30;s.spawn=1.3;s.boss=32;s.nextSpecial=12;s.specialIndex=0;s.nextFlockId=1;s.mainShotCount=0;s.wingRecruitTimer=45;
   const bosses=['dinosauria','phoenix','morpho'];s.bossModel=bosses.includes(options.bossModel)?options.bossModel:'dinosauria';s.bossIndex=bosses.indexOf(s.bossModel);s.bossSpawnCount=0;s.bossCap=1;s.wave=1;s.waveKind='normal';s.wavePhase='engaging';s.waveElapsed=0;s.waveSpawned=0;s.waveTotal=wavePlan(1).total;s.waveBossSpawned=0;s.waveEliteSpawned=0;s.regroupRemaining=0;s.threatStage=1;s.eliteQueue=[];s.eliteIndex=0;
  ...
  spawnEnemy(s,type='normal',x,y){
   ...
   if(type!=='boss'&&living.filter(e=>e.type!=='boss').length>=65)return null;
   if(['normal','gunner'].includes(type)&&living.filter(e=>['normal','gunner'].includes(e.type)).length>=24)return null;
   if(['scout','jammer','mine','swarm'].includes(type)&&living.filter(e=>['scout','jammer','mine','swarm'].includes(e.type)).length>=16)return null;
   if(['charger','artillery','shield','stier'].includes(type)&&living.filter(e=>e.type===type).length>=Math.min(5,2+Math.floor(((s.threatStage||1)-1)/2)))return null;
   ...
   const wave=Math.max(1,s.wave||1),early=wave<4?.75:wave<7?.9:1,spec={charger:[360,25,32,128],artillery:[420,32,36,70],scout:[100,8,22,115],shield:[700,48,42,55],jammer:[120,9,22,95],mine:[70,6,20,120],swarm:[28,3,12,135],stier:[750,70,34,65],gunner:[55,4,20,90]}[type];
   const boss=type==='boss',hp=boss?2200:spec?spec[0]+spec[1]*(wave-1):32+3*(wave-1);
   const e={id:s.nextId++,type,x,y,side,hp,max:hp,r:boss?105:spec?spec[2]:18,
    speed:boss?42:spec?spec[3]:82+Math.min(70,s.time*.3),dead:false,flash:0,walk:0,
    angle:Math.atan2(s.y-y,s.x-x),cooldown:type==='artillery'?2.4:.5,contactCooldown:0,
    phase:'approach',windup:0,lockedX:null,lockedY:null,stagger:0,attackCount:0,nodes:[],exposed:0,heat:0,cooling:0,damageScale:early*(1+(wave-1)*.02),jamRadius:type==='jammer'?240:type==='swarm'?170:0,flying:['jammer','swarm'].includes(type)};
  ...
  spawnSpecial(s,type,x,y){
   if(['dinosauria','phoenix','morpho'].includes(type)){
    ...
    if(!e)return null;
    const wave=Math.max(1,s.wave||1),bossHp={dinosauria:[2800,260],phoenix:[2800,260],morpho:[3200,300]}[type];e.model=type;e.hp=e.max=bossHp[0]+bossHp[1]*(wave-1);e.r=type==='phoenix'?48:105;e.speed=type==='phoenix'?240:42;e.cooldown=2.2;e.intro=1.2;e.bossStage=1;
  ...
  collect(s,box){
   ...
   if(box.kind==='repair'){s.hp=Math.min(s.maxHp,s.hp+12);for(const wing of syncWings(s))wing.hp=Math.min(wing.maxHp,wing.hp+12);}
   else if(box.kind==='shield')s.shield=Math.min(s.maxHp,s.shield+10);
   else if(box.kind==='charge')s.charges=Math.min(2,s.charges+1);
   else if(box.kind==='recruit'){const count=s.count;if(s.count<s.maxCount){s.count++;syncWings(s);s.stats.recruits++;}else for(const wing of syncWings(s))wing.hp=Math.min(wing.maxHp,wing.hp+24);}
    ...
    for(const e of s.enemies)if(!e.dead&&Math.hypot(e.x-box.x,e.y-box.y)<=230+e.r){
     if(e.model==='phoenix')e.revealed=Math.max(e.revealed||0,3.5);
     ...
     else if(e.model==='phoenix'&&['stalk','closing','flank','windup','slashWindup','dash'].includes(e.phase)){e.phase='recover';e.windup=0;e.dashLeft=0;e.exposed=Math.max(e.exposed,1.8);s.events.push({type:'phoenixRecover',x:e.x,y:e.y,sourceId:e.id,exposed:1.8,interrupted:true});}
  ...
  damageEnemy(s,e,damage,cause={}){
   ...
   e.hp=0;e.dead=true;if(e.retreating)return before;s.kills++;
   const identity={sourceId:e.id,kind:e.type,model:e.model||e.type,x:e.x,y:e.y,angle:e.angle};
   s.events.push({type:'kill',...identity});
   if(e.type==='boss'){
    s.bossKills++;s.bossWarned=false;
   s.events.push({type:'bossDown',...identity});
    {const first=pickupSpot(s,e.x-45,e.y),second=pickupSpot(s,e.x+45,e.y);s.crates.push({id:s.nextId++,kind:'weapon',x:first.x,y:first.y,used:false,drop:true,premium:true,life:30},{id:s.nextId++,kind:dropKind(s,'boss'),x:second.x,y:second.y,used:false,drop:true,premium:true,life:30});}
   }else if(!['charger','artillery','shield','stier'].includes(e.type)){
    ...
    {const spot=pickupSpot(s,e.x,e.y);s.crates.push({id:s.nextId++,kind:dropKind(s,'elite'),x:spot.x,y:spot.y,used:false,drop:true,premium:true,life:30});}
  ...
  useTactic(s,aim){
   const blockers=s.enemies.filter(e=>!e.dead&&e.jamRadius&&Math.hypot(e.x-s.x,e.y-s.y)<=e.jamRadius);s.uplinkBlocked=blockers.length>0;
   if(s.over||!s.charges||s.tacticCooldown>0)return false;
   if(s.uplinkBlocked){s.events.push({type:'tacticBlocked',kind:'support',x:s.x,y:s.y,sourceIds:blockers.map(e=>e.id)});return false;}
  ...
  useBurst(s,input={}){
   ...
   for(const e of s.enemies)if(!e.dead&&Math.hypot(e.x-s.x,e.y-s.y)<=radius+e.r){const damage=C.damageEnemy(s,e,e.type==='boss'?(blade?120:80):(blade?240:160));if(damage){let interrupted=false;if(!e.dead){e.stagger=Math.max(e.stagger,blade?1:.8);if(blade){for(const h of s.hazards)if(!h.fired&&(h.source===e.id||h.linkedTo===e.id))h.cancelled=interrupted=true;if(e.type==='charger'){e.phase='stagger';e.windup=0;e.lockedX=e.lockedY=null;interrupted=true;}else if(e.model==='phoenix'&&['stalk','closing','flank','windup','slashWindup','dash'].includes(e.phase)){e.phase='recover';e.windup=0;e.dashLeft=0;e.exposed=Math.max(e.exposed,1.8);s.events.push({type:'phoenixRecover',x:e.x,y:e.y,sourceId:e.id,exposed:1.8,interrupted:true});interrupted=true;}}}hits.push({id:e.id,damage,killed:e.dead,interrupted});}}
  ...
  insideHazard(s,h){
   return insideShape(s.x,s.y,0,h)&&!(h.geometry==='sector'&&obstacleHit(s,h.x,h.y,s.x,s.y,0));
  },
  selectWeapon(s,slot){
  ...
  fire(s,machine){
   ...
   if(s.weaponSlot>1){
    ...
    const level=s.level,damage=kind==='autocannon'?18+1.8*Math.min(12,level)+1.4*Math.max(0,level-12):120+5*Math.min(12,level)+2.5*Math.max(0,level-12),left=spec.left;
    s.bullets.push({x:s.x,y:s.y,px:s.x,py:s.y,vx:dx/length*spec.speed,vy:dy/length*spec.speed,r:spec.r,damage,enemy:false,life:spec.range/spec.speed,shell:true,ammo:kind,left,hitIds:[],special:true});s.specialAmmo[kind]--;s.shot=spec.interval;
    s.events.push({type:'specialShot',slot,kind,x:s.x,y:s.y,angle:s.angle,damage,remaining:s.specialAmmo[kind],targetId:target.id});
   ...
   const damage=(machine.damage+2.5*Math.min(12,level)+Math.max(0,level-12))*ammoDamage;s.mainShotCount=(s.mainShotCount||0)+1;
  ...
  updateEnemy(s,e,dt){
   ...
   if(e.retreating){const wb=world(s),left=e.x-wb.left,right=wb.right-e.x,top=e.y-wb.top,bottom=wb.bottom-e.y,min=Math.min(left,right,top,bottom),speed=Math.max(220,e.speed||0);if(min===left)e.x-=speed*dt;else if(min===right)e.x+=speed*dt;else if(min===top)e.y-=speed*dt;else e.y+=speed*dt;if(e.x<wb.left-e.r||e.x>wb.right+e.r||e.y<wb.top-e.r||e.y>wb.bottom+e.r)e.dead=true;return;}
   const hadStagger=e.stagger>0;e.stagger=Math.max(0,e.stagger-dt);
   let target=s.decoy||s;
   if(s.campaign&&e.type==='shield'&&!s.decoy){const scout=s.enemies.find(other=>other.type==='scout'&&!other.dead),near=Math.hypot(s.x-e.x,s.y-e.y)<=330;if(scout)e.lastObserved={x:s.x,y:s.y};target=scout||near?s:(e.lastObserved||{x:e.x,y:e.y});}
   const routed=routeTarget(s,e,target),dx=routed.x-e.x,dy=routed.y-e.y,d=Math.hypot(dx,dy)||1;e.angle=Math.atan2(target.y-e.y,target.x-e.x);
   ...
   if(e.type==='shield'){
    if(e.campaignSentry){if(!e.campaignActive&&(s.campaign.cargo||Math.hypot(s.x-e.x,s.y-e.y)<=700)){e.campaignActive=true;s.events.push({type:'sentryActive',sourceId:e.id,x:e.x,y:e.y});}if(!e.campaignActive)return;}
    ...
    if(!e.campaignSentry&&Math.hypot(target.x-e.x,target.y-e.y)>330)moveCircle(s,e,dx/d*e.speed*dt,dy/d*e.speed*dt);return;
   }
   if(e.type==='stier'){
   ...
   if(e.type==='boss'){
    if(!e.bossStage)e.bossStage=1;if(s.wave>6&&e.bossStage<2&&(e.hp<=e.max*.5||(s.threatStage||1)>=2)){e.bossStage=2;s.events.push({type:'bossPhase',sourceId:e.id,model:e.model,phase:2});}
    ...
    const pendingBosses=preparing.size,prepareCap=s.wave>=19?3:2;
    if(e.model==='phoenix'){
     e.intro=Math.max(0,(e.intro||0)-dt);
     if(e.phase==='recover'){if(e.exposed<=0){e.phase='approach';e.cooldown=.8;}return;}
     const recover=(duration=1.8)=>{e.phase='recover';e.windup=0;e.dashLeft=0;e.recoverDuration=duration;e.exposed=Math.max(e.exposed,duration);e.cooldown=.8;s.events.push({type:'phoenixRecover',x:e.x,y:e.y,sourceId:e.id,exposed:duration});};
     const slash=()=>{const angle=Math.atan2(target.y-e.y,target.x-e.x);e.phase='slashWindup';e.windup=1.1;e.revealed=Math.max(e.revealed||0,1.4);C.addHazard(s,'sector',e,'blade',{x:e.x,y:e.y,r:165,angle,halfAngle:Math.PI/3,heavy:true},1.1+dt,22*e.damageScale);s.hazards.at(-1).maxDelay=1.1;s.events.push({type:'phoenixSlashCue',x:e.x,y:e.y,radius:165,angle,halfAngle:Math.PI/3,delay:1.1,sourceId:e.id,combo:2});};
     if(e.phase==='stalk'){
      e.stalkTime-=dt;const actual=Math.hypot(target.x-e.x,target.y-e.y)||1,travelX=routed.x-e.x,travelY=routed.y-e.y,travel=Math.hypot(travelX,travelY)||1,side=e.attackCount%2?1:-1,pace=actual>420?1.65:1.15;
      moveCircle(s,e,(travelX/travel*pace-(target.y-e.y)/actual*.38*side)*e.speed*dt,(travelY/travel*pace+(target.x-e.x)/actual*.38*side)*e.speed*dt,false);
      if(e.stalkTime<=0&&!obstacleHit(s,e.x,e.y,target.x,target.y,0)){const angle=Math.atan2(target.y-e.y,target.x-e.x),distance=clamp(Math.hypot(target.x-e.x,target.y-e.y)+80,220,520),delay=1;e.phase='windup';e.windup=delay;e.revealed=Math.max(e.revealed||0,delay+.05);e.lockedX=e.x+Math.cos(angle)*distance;e.lockedY=e.y+Math.sin(angle)*distance;C.addHazard(s,'beam',e,'charge',{x:e.lockedX,y:e.lockedY,width:2*(e.r+16)},delay+dt,0);s.hazards.at(-1).maxDelay=delay;s.events.push({type:'phoenixCue',x:e.x,y:e.y,toX:e.lockedX,toY:e.lockedY,sourceId:e.id,delay});}else if(e.stalkTime<=0)e.stalkTime=.35;return;
     }
     ...
     if(e.phase==='dash'){
      const tx=e.lockedX-e.x,ty=e.lockedY-e.y,left=Math.hypot(tx,ty),early=s.wave<=3,move=Math.min(left,e.dashLeft,(early?420:560)*dt),beforeX=e.x,beforeY=e.y;
      ...
      const touched=Math.hypot(s.x-e.x,s.y-e.y)<=e.r+22+.5;if(touched&&e.contactCooldown<=0){C.hurt(s,(early?24:30)*e.damageScale,false,{kind:'ram',model:'phoenix',sourceId:e.id});e.contactCooldown=1;}
      if(touched)recover(early?2.2:1.8);else if(moved<move*.6||e.dashLeft<=1||left<=1){if(s.wave>=7||s.wave>=4&&e.hp<=e.max*.5)slash();else recover(early?2.2:1.8);}return;
     }
     ...
     e.cooldown-=dt;const actual=Math.hypot(target.x-e.x,target.y-e.y)||1,travelX=routed.x-e.x,travelY=routed.y-e.y,travel=Math.hypot(travelX,travelY)||1;
     if(actual>260)moveCircle(s,e,travelX/travel*e.speed*1.2*dt,travelY/travel*e.speed*1.2*dt,false);
     if(e.intro<=0&&e.cooldown<=0){if(pendingBosses>=prepareCap){e.cooldown=.5;return;}e.attackCount++;s.stats.bossShots++;e.phase='stalk';e.stalkTime=1.8;e.revealed=0;s.events.push({type:'phoenixStalk',x:e.x,y:e.y,sourceId:e.id,duration:1.8});}return;
    }
    ...
    if(e.intro<=0&&e.cooldown<=0){if(pendingBosses>=prepareCap){e.cooldown=.5;return;}
     e.attackCount++;e.cooldown=s.wave<=6?4.8:e.hp<e.max*.45?2.8:3.7;
     if(e.model==='dinosauria'){
      const base=Math.atan2(target.y-e.y,target.x-e.x);
      if(s.wave<=6&&e.attackCount%2)C.addHazard(s,'circle',e,'mortar',{x:target.x,y:target.y,r:150,exposeAfter:true},1.15,40*e.damageScale);
      else{if(s.wave>6)C.addHazard(s,'circle',e,'mortar',{x:target.x,y:target.y,r:150},1.15,40*e.damageScale);for(const spread of [-.32,0,.32])C.addHazard(s,'beam',e,'rail',{x:e.x+Math.cos(base+spread)*900,y:e.y+Math.sin(base+spread)*900,width:38,heavy:true,exposeAfter:s.wave<=6||e.bossStage<2&&spread===.32},1.3,48*e.damageScale);if(e.bossStage>=2)C.addHazard(s,'circle',e,'mortar',{x:clamp(target.x+Math.cos(base+Math.PI/2)*170,s.bounds.left,s.bounds.right),y:clamp(target.y+Math.sin(base+Math.PI/2)*170,s.bounds.top,s.bounds.bottom),r:105,exposeAfter:true},1.7,26*e.damageScale);}
     }else{
      const side=e.attackCount%2?1:-1;
      if(s.wave<=6){if(e.attackCount%2)C.addHazard(s,'beam',e,'rail',{x:target.x+(target.x-e.x)*2,y:target.y+(target.y-e.y)*2,width:58,heavy:true,exposeAfter:true},1.5,60*e.damageScale);else C.addHazard(s,'circle',e,'mortar',{x:target.x,y:target.y,r:82,exposeAfter:true},1.75,28*e.damageScale);}
      else{C.addHazard(s,'beam',e,'rail',{x:target.x+(target.x-e.x)*2,y:target.y+(target.y-e.y)*2,width:58,heavy:true},1.5,60*e.damageScale);C.addHazard(s,'circle',e,'mortar',{x:clamp(target.x+side*105,s.bounds.left,s.bounds.right),y:target.y,r:82},1.75,28*e.damageScale);C.addHazard(s,'circle',e,'mortar',{x:clamp(target.x-side*105,s.bounds.left,s.bounds.right),y:target.y,r:82,exposeAfter:e.bossStage<2},2.15,28*e.damageScale);if(e.bossStage>=2)C.addHazard(s,'beam',e,'rail',{x:target.x-(target.y-e.y),y:target.y+(target.x-e.x),width:36,exposeAfter:true},2.45,26*e.damageScale);}
     }
   ...
   if(e.type==='artillery'){
    ...
    e.cooldown-=dt;if(e.cooldown<=0){e.cooldown=3.8;C.addHazard(s,'circle',e,'mortar',{x:target.x,y:target.y,r:78},1.1,24*e.damageScale);const scout=s.enemies.find(n=>n.type==='scout'&&!n.dead&&Math.hypot(n.x-e.x,n.y-e.y)<260);if(scout){const toX=clamp(target.x+(scout.x-e.x)*.45,s.bounds.left,s.bounds.right),toY=clamp(target.y+(scout.y-e.y)*.45,s.bounds.top,s.bounds.bottom);C.addHazard(s,'circle',e,'mortar',{x:toX,y:toY,r:68},1.55,20*e.damageScale);s.hazards.at(-1).linkedTo=scout.id;s.events.push({type:'spotterLink',sourceId:e.id,spotterId:scout.id,x:e.x,y:e.y,toX,toY});}s.stats.eliteShots++;s.events.push({type:'warning',kind:'mortar',x:target.x,y:target.y,linkedTo:scout?.id});}
    return;
   }
   if(e.type==='charger'){
  ...
  step(s,dt,input={}){
   ...
   if(s.wavePhase==='regroup'){s.regroupRemaining=Math.max(0,s.regroupRemaining-dt);if(s.regroupRemaining<=1e-9){const oldThreat=s.threatStage;s.wave++;s.level++;const plan=wavePlan(s.wave);s.waveKind=plan.kind;s.wavePhase='engaging';s.waveElapsed=0;s.waveSpawned=0;s.waveTotal=plan.total;s.waveBossSpawned=0;s.waveEliteSpawned=0;s.spawn=1.3+dt;s.threatStage=1+Math.floor((s.wave-1)/3);s.bossCap=bossCapForWave(s.wave);s.events.push({type:'waveStart',wave:s.wave,kind:plan.kind,threatStage:s.threatStage,total:plan.total});if(s.threatStage!==oldThreat)s.events.push({type:'threatStage',stage:s.threatStage,bossCap:s.bossCap});if(s.level>=12&&s.level%4===0)s.events.push({type:'weaponMilestone',level:s.level,tier:(s.level-8)/4});}}else s.waveElapsed+=dt;
   s.waveKind=waveKind(s.wave);s.bossCap=bossCapForWave(s.wave);
   ...
   if(s.charges<2){s.tacticRecharge-=dt;if(s.tacticRecharge<=1e-9){s.charges++;s.tacticRecharge=30;s.events.push({type:'tacticRecharge',charges:s.charges,cooldown:30});}}else s.tacticRecharge=30;
   ...
   {const sourceIds=s.enemies.filter(e=>!e.dead&&e.jamRadius&&Math.hypot(e.x-s.x,e.y-s.y)<=e.jamRadius).map(e=>e.id),blockedNow=sourceIds.length>0;if(blockedNow!==s.uplinkBlocked)s.events.push({type:'uplinkChanged',blocked:blockedNow,sourceIds});s.uplinkBlocked=blockedNow;}
   for(const h of s.hazards){
    ...
    if(!h.fired){h.delay-=dt;if(h.delay<=1e-9){h.fired=true;if(h.exposeAfter){const source=s.enemies.find(e=>e.id===h.source&&!e.dead);if(source)source.exposed=2;}
     if(h.kind==='support'){const hits=[];for(const e of s.enemies)if(!e.dead&&Math.hypot(e.x-h.x,e.y-h.y)<=h.r+e.r){const boss=e.type==='boss',exposed=boss&&e.exposed>0,elite=['charger','artillery','shield','stier'].includes(e.type);if(e.model==='phoenix')e.revealed=Math.max(e.revealed||0,3.5);const damage=C.damageEnemy(s,e,boss?(exposed?300:120):elite?220:450);if(damage){let interrupted=false;if(!e.dead&&(!boss||exposed)){e.stagger=Math.max(e.stagger,.8);for(const pending of s.hazards)if(!pending.fired&&(pending.source===e.id||pending.linkedTo===e.id))pending.cancelled=interrupted=true;if(e.type==='charger'){e.phase='stagger';e.windup=0;e.lockedX=e.lockedY=null;interrupted=true;}else if(e.type==='stier'){e.phase='cooling';e.windup=0;e.cooling=.8;interrupted=true;}else if(e.model==='phoenix'&&['stalk','windup','slashWindup','dash'].includes(e.phase)){e.phase='recover';e.windup=0;e.dashLeft=0;s.events.push({type:'phoenixRecover',x:e.x,y:e.y,sourceId:e.id,exposed:e.exposed,interrupted:true});interrupted=true;}else interrupted=true;s.stats.interrupts++;}hits.push({id:e.id,damage,killed:e.dead,interrupted});}}s.events.push({type:'mortarImpact',kind:h.kind,x:h.x,y:h.y,r:h.r,sourceId:h.source,model:h.model,hits});}
    ...
    if(h.fired&&!h.playerImmune&&h.damage>0){h.hitWingIds=h.hitWingIds||[];for(const f of C.formation(s).slice(1))if(!h.hitWingIds.includes(f.wingId)&&insideShape(f.x,f.y,12,h)&&!(h.geometry==='sector'&&obstacleHit(s,h.x,h.y,f.x,f.y,0))){h.hitWingIds.push(f.wingId);C.damageWing(s,f.wingId,h.damage*(h.geometry==='beam'?1:.6),{kind:h.kind,sourceId:h.source});}}
   }
   ...
   if(s.count<=1)s.wingRecruitTimer=Math.max(0,(s.wingRecruitTimer??45)-dt);else s.wingRecruitTimer=45;
   if(!s.campaign){s.pickupTimer-=dt;if(s.pickupTimer<=1e-9){s.pickupTimer=18;const pairs=[[[160,180],[290,180]],[[500,820],[630,820]],[[440,590],[570,590]],[[820,520],[820,650]]],anchors=s.supplyAnchors,positions=anchors?[anchors[s.supplyGroup%anchors.length],anchors[(s.supplyGroup+1)%anchors.length]]:pairs[s.supplyGroup%pairs.length],offense=['charge','overdrive','emp','recruit','ap','he'],survival=s.wingRecruitTimer<=0?'recruit':s.supplyGroup%2?'repair':'shield',attack=s.supplyGroup===0?'autocannon':s.supplyGroup===1?'heavyCannon':offense[Math.floor(s.random()*offense.length)],kinds=[survival,attack],group=++s.supplyGroup;if(survival==='recruit')s.wingRecruitTimer=45;for(const [i,p] of positions.entries()){const spot=pickupSpot(s,p[0],p[1]);s.crates.push({id:s.nextId++,group,kind:kinds[i],x:spot.x,y:spot.y,used:false,life:18});}s.events.push({type:'supplyChoice',group,crates:s.crates.filter(b=>b.group===group).map(b=>({id:b.id,kind:b.kind,x:b.x,y:b.y}))});}}
   if(!s.campaign&&s.wavePhase==='engaging'&&s.waveSpawned<s.waveTotal){s.spawn-=dt;if(s.spawn<=0){s.spawn=Math.max(.7,1.3-Math.max(0,s.wave-3)*.04);const plan=wavePlan(s.wave),eliteTypes=['charger','artillery','shield','stier'],smallTypes=['scout','jammer','mine','swarm'];let spawned=0;
     if(s.waveBossSpawned<plan.bosses){const bosses=['dinosauria','phoenix','morpho'],e=C.spawnSpecial(s,bosses[(s.bossIndex+s.bossSpawnCount)%bosses.length]);if(e){s.bossSpawnCount++;s.waveBossSpawned++;spawned=1;}}
     else if(s.waveEliteSpawned<plan.elites){for(let attempt=0;attempt<eliteTypes.length&&!spawned;attempt++){const index=(s.eliteIndex+attempt)%eliteTypes.length,e=C.spawnEnemy(s,eliteTypes[index]);if(e){s.eliteIndex=(index+1)%eliteTypes.length;s.waveEliteSpawned++;spawned=1;}}}
     else {const roll=s.random(),limit=s.waveTotal-s.waveSpawned;let type=roll<.25?smallTypes[s.specialIndex++%smallTypes.length]:s.time>=12&&s.random()<.32?'gunner':'normal';if(type==='swarm'&&limit<6)type='scout';if(type==='swarm'){const flockId=s.nextFlockId++,leader=C.spawnEnemy(s,'swarm'),flock=[];if(leader){leader.flockId=flockId;leader.flockLeader=true;flock.push(leader);spawned=1;for(let member=1;member<6;member++){let fly=null;for(const radius of [44,64,84,104,124])for(let step=0;step<16&&!fly;step++){const angle=(step+member*3)*Math.PI/8,x=leader.x+Math.cos(angle)*radius,y=leader.y+Math.sin(angle)*radius;if(flock.every(other=>Math.hypot(x-other.x,y-other.y)<=160)&&!blocked(s,{r:12},x,y,true))fly=C.spawnEnemy(s,'swarm',x,y);}if(fly){fly.flockId=flockId;fly.flockLeader=false;flock.push(fly);spawned++;}}}}else if(C.spawnEnemy(s,type))spawned=1;}
     s.waveSpawned+=spawned;if(s.waveSpawned>=s.waveTotal)s.wavePhase='cleanup';}}
   s.enemies=s.enemies.filter(e=>!e.dead);
   if(!s.campaign&&!s.over&&s.wavePhase==='cleanup'){const living=s.enemies.filter(e=>!e.dead&&!e.retreating),major=living.filter(e=>e.type==='boss'||['charger','artillery','shield','stier'].includes(e.type));if(!major.length&&living.length<=2){for(const e of living)e.retreating=true;const hp=s.hp,shield=s.shield;s.hp=Math.min(s.maxHp,s.hp+8);s.shield=Math.min(s.maxHp,s.shield+10);for(const wing of syncWings(s))wing.hp=Math.min(wing.maxHp,wing.hp+8);s.bullets=s.bullets.filter(b=>!b.enemy);s.hazards=s.hazards.filter(h=>h.playerImmune);s.wavePhase='regroup';s.regroupRemaining=4;s.events.push({type:'waveCleared',wave:s.wave,hpRecovered:s.hp-hp,shieldRecovered:s.shield-shield,duration:4});}}
