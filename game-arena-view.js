...
function processArenaEvents(){
 for(const e of s.events){
  ...
  else if(e.type==='impact'){if(e.damage<=0)continue;if(e.shell&&!e.killed&&!hitResponses.some(fx=>fx.id===e.targetId&&fx.age<.09)){hitResponses.push({id:e.targetId,age:0,power:e.ammo==='ap'?1:.8});hitResponses=hitResponses.slice(-12);}if(!e.killed)metalImpact(e.x,e.y,e.armored);beep(e.ammo==='ap'?'apImpact':'impact',{intensity:.65});}
  else if(e.type==='kill'){const heavy=!['normal','gunner','swarm','mine'].includes(e.kind),duration=e.kind==='boss'?3:heavy?1.6:.55;const smokers=wrecks.filter(w=>w.smoke),eligible=['boss','stier','charger','shield','artillery'].includes(e.kind);let smoke=eligible&&smokers.length<4;if(eligible&&e.kind==='boss'&&!smoke){const older=smokers.find(w=>w.kind!=='boss')||smokers[0];if(older){older.smoke=false;smoke=true;}}wrecks.push({...e,id:e.sourceId,type:e.kind,recoil:0,walk:0,phase:0,life:duration,duration,burstStage:0,smoke});wrecks=wrecks.slice(-30);if(!heavy)blastEffect(e.x,e.y,42);metalImpact(e.x,e.y,false,heavy);}
  ...
  else if(e.type==='threatStage'){if(s.waveKind==='normal')eventNotice('THREAT LEVEL',`戰場升階 · ${e.stage}`,`重型目標上限 ${e.bossCap}`,2.4);beep('warning');battleRadio('arena-stage-'+e.stage,'敵方增援升級。保留支援，別讓重型目標合圍。','warning','lena',{priority:4,duration:3});}
  ...
  else if(e.type==='tacticBlocked'){beep('warning');battleRadio('arena-uplink-blocked','支援上行受阻。先離開飛行器干擾範圍。','warning','lena',{priority:5,duration:2.4,valid:()=>s.uplinkBlocked});}
  else if(e.type==='uplinkChanged'&&!e.blocked){beep('braceReady');}
  ...
  else if(e.type==='phoenixStalk'){battleRadio('phoenix-stalk','訊號消失了。保持移動，留意側翼。','warning','shin',{priority:4,duration:1.6,valid:()=>s.enemies.some(enemy=>enemy.id===e.sourceId&&!enemy.dead&&enemy.phase==='stalk'&&!(enemy.revealed>0))});}
  else if(e.type==='phoenixCue'){beep('chargerReady');battleRadio('phoenix-lock','顯形了！離開鎖定路徑。','warning','shin',{priority:5,duration:2.8});}
...
function drawArenaUnit(entity,index,size,allied=false,dead=false){
 if(!dead&&entity.model==='phoenix'&&entity.phase==='stalk'&&!(entity.revealed>0))return;
...
function drawArenaEnemyState(e,size){
 if(e.model==='phoenix'&&e.phase==='stalk'&&!(e.revealed>0))return;
 ...
 if(open){
  const duration=e.type==='stier'?2.4:e.model==='phoenix'?(e.recoverDuration||(s.wave<=3?2.2:1.8)):2,release=clamp(e.exposed/duration,0,1),opening=motion?clamp((duration-e.exposed)/.12,0,1):1,travel=motion?(1-release)*size*.12:0;ctx.globalAlpha=.75*opening;
  for(const side of [-1,1])for(let i=0;i<3;i++){const x=size*(-.12+i*.07),y=side*size*.10;line({x,y},{x:x-size*.045,y:y+side*size*.05},'#142125',5);line({x,y},{x:x-size*.045,y:y+side*size*.05},e.model==='phoenix'?'#e9a17a':'#edcb99',2);}
 ...
 if(e.model==='phoenix'&&e.phase==='windup'){const reveal=clamp(1-progress/.5,0,1);ctx.globalAlpha=motion?reveal*.6:.35;for(const side of [-1,1]){line({x:-size*.28,y:side*size*.2},{x:-size*.12,y:side*size*.29},'#a3c4ce',1.5);line({x:size*.08,y:side*size*.31},{x:size*.25,y:side*size*.23},'#d2e2df',1.5);}}
...
function drawArenaBladeSector(hazard,outlineOnly=false){
 ...
 ctx.save();ctx.beginPath();ctx.moveTo(p.x,p.y);ctx.ellipse(p.x,p.y,rx,ry,0,start,end);ctx.closePath();
 if(outlineOnly){ctx.strokeStyle='#07131fcc';ctx.lineWidth=5;ctx.stroke();ctx.strokeStyle=pending?'#efc09b':'#f4e5cf';ctx.lineWidth=2;ctx.stroke();ctx.restore();return;}
 ctx.fillStyle=pending?'#d5b39325':'#d5b39324';ctx.fill();ctx.strokeStyle=color;ctx.lineWidth=1.4;ctx.stroke();ctx.clip();
 if(pending){
...
function drawArenaScene(){
 ...
 for(const hazard of s.hazards)if(!hazard.cancelled&&hazard.geometry==='sector')drawArenaBladeSector(hazard,true);
