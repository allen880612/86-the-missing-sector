...
- **四向無限**：WASD／方向鍵移動、滑鼠瞄準、主砲自動開火，僚機自主迎擊附近敵人。左鍵火控超頻；右鍵0.36秒越障躍進，M1A4／XM2／M4A3各180／220／150單位，冷卻3秒；空中免接觸衝撞，砲擊仍可命中；Space在游標位置呼叫區域支援，初始2次、每18秒回充1次。P暫停。
...
v34：191項遊戲／音訊／部署測試通過。新增Phönix高機動Boss、Stier伏擊砲兵菁英與Ameise掃射班；四向首Boss可選、區域支援可靠回充，通訊與介面補進退場動畫。受控Chrome驗證新機體、連續突襲／撲空窗口、三線射界／支援打斷與結算；世界觀依據與遊戲改編見[新機體素材說明](assets/legion-v34.md)。音樂聽感與長局平衡持續收集玩家回饋，不宣稱所有硬體穩定60FPS。

四向普通敵每12隻擊破掉落，菁英掉落間隔至少8秒；定時補給25秒（首次8秒），Boss仍必掉。滿LV8拾取火控改為至少5秒火力全開，不疊乘倍率。僚機在扣盾後致命傷時犧牲攔截，保留主機HP，6秒內不重複；結算記錄次數。
